...
export default class App extends React.Component {
  ...
  componentDidMount(): void {
    ...
    onPingDidUpdateListener((state) => {
      console.log(`ping.onPingDidUpdateListener--->${state.keys}--->${state.values}`);
        ...
        renderItem={(info) => {
          return (
            <TouchableOpacity onPress={() => {
              ...
              <Text style={{ height: 100, textAlign: 'center', color: "blue", backgroundColor: "yellow", verticalAlign: "middle" }}>{info.item}</Text>
            </TouchableOpacity>
          )
        }}></FlatList>
    )
